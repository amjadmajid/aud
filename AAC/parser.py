...
def parse_subchirp_file_mt(file: str, transmittor_count: int, files: list) -> (str, int, float, float, int, float):
    ...
    cycles: float = get_cycles({"configuration": conf, "symbol_time": Ts, "fstart": fstart, "fend": fend})

    encoder = OChirpEncode(T=None, fs=fstart, fe=fend, T_preamble=0, orthogonal_pair_offset=offset,
    ...
    if transmittor_count > 1:
        ...
        for possible_config in possible_noise_files:
            ...
            if (f"\\{conf}\\" in possible_config or f"/{conf}/" in possible_config) and \
